...
from db import get_user_by_id, add_user
import re
from aiogram.fsm.context import FSMContext
from utils.states import UserData
    ...
    if user_data is None:
        if telegram_id in admin_ids:
            await message.answer(
                f"<b>{message.from_user.first_name}</b>\nДобро пожаловать!\n\nНапоминаю, что вы являетесь администратором\nПеред началом работы с ботом необходимо зарегестрироваться",
...
async def form_phone(message:Message, state:FSMContext):
    ...
    if phone_pattern.match(message.text):
        ...
        await message.answer(f"Ваш номер телефона {phone_number} сохранён")
